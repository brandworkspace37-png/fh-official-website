...
async function saveProjectLead(request, env) {
  ...
  const lead = normalizeProjectLead(await request.json()), customerId = crypto.randomUUID(), leadId = crypto.randomUUID(), now = new Date().toISOString();
  const existing = await env.DB.prepare("SELECT id FROM customers WHERE lower(email) = lower(?) LIMIT 1").bind(lead.email).first("id"), resolvedCustomerId = existing?.id || customerId, interestsJson = JSON.stringify(lead.interests), metadata = JSON.stringify({ lead_id: leadId, country: lead.country, zip: lead.zip, interests: lead.interests, details: lead.details, attachment: lead.attachment, source: "website", submitted_at: now });
  const statements = [];
  if (existing?.id) statements.push(env.DB.prepare("UPDATE customers SET first_name = ?, phone = ?, language = ?, updated_at = datetime('now') WHERE id = ?").bind(lead.name, lead.phone, lead.country === "US" ? "en" : "es", resolvedCustomerId)); else statements.push(env.DB.prepare("INSERT INTO customers (id, first_name, last_name, email, phone, language, status) VALUES (?, ?, ?, ?, ?, ?, 'active')").bind(resolvedCustomerId, lead.name, "", lead.email, lead.phone, lead.country === "US" ? "en" : "es"));
  ...
  await env.DB.batch(statements); return { success: true, leadId, customerId: resolvedCustomerId };
}
async function getAccessToken(env) {
  const clientId = env.PAYPAL_CLIENT_ID || PAYPAL_SANDBOX_CLIENT_ID; if (!clientId || !env.PAYPAL_CLIENT_SECRET) throw new Error("PayPal server secret is not configured.");
  const credentials = btoa(`${clientId}:${env.PAYPAL_CLIENT_SECRET}`), response = await fetch(`${PAYPAL_BASE}/v1/oauth2/token`, { method: "POST", headers: { Authorization: `Basic ${credentials}`, "Content-Type": "application/x-www-form-urlencoded", Accept: "application/json" }, body: "grant_type=client_credentials" }), data = await response.json();
  if (!response.ok || !data.access_token) { console.error("PayPal OAuth error", response.status, data); throw new Error("Unable to authenticate with PayPal."); } return data.access_token;
}
function normalizeCart(cart) {
  if (!Array.isArray(cart) || cart.length < 1 || cart.length > 20) throw new Error("Invalid cart."); let subtotal = 0; const items = [];
...
async function createPayPalOrder(request, env) {
  if (!env.DB) throw new Error("Database binding is not configured."); const body = await request.json(), { subtotal, items } = normalizeCart(body.cart), customer = body.customer || {};
  ...
  const response = await fetch(`${PAYPAL_BASE}/v2/checkout/orders`, { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json", Accept: "application/json", Prefer: "return=representation", "PayPal-Request-Id": crypto.randomUUID() }, body: JSON.stringify(payload) }), data = await response.json(); if (!response.ok || !data.id) { console.error("PayPal create order error", response.status, data); throw new Error("PayPal could not create the payment order."); }
...
async function capturePayPalOrder(request, env) {
  if (!env.DB) throw new Error("Database binding is not configured."); const orderId = String((await request.json()).orderID || "").trim(); if (!/^[A-Z0-9]+$/.test(orderId)) throw new Error("Invalid PayPal order ID.");
  const token = await getAccessToken(env), orderResponse = await fetch(`${PAYPAL_BASE}/v2/checkout/orders/${encodeURIComponent(orderId)}`, { headers: { Authorization: `Bearer ${token}`, Accept: "application/json" } }), order = await orderResponse.json(); if (!orderResponse.ok || order.status !== "APPROVED") throw new Error("The PayPal order is not approved for capture.");
  const customId = order.purchase_units?.[0]?.custom_id || ""; if (!customId.startsWith("FH-")) throw new Error("Unrecognized merchant order.");
  const captureResponse = await fetch(`${PAYPAL_BASE}/v2/checkout/orders/${encodeURIComponent(orderId)}/capture`, { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json", Accept: "application/json", Prefer: "return=representation", "PayPal-Request-Id": crypto.randomUUID() }, body: "{}" }), capture = await captureResponse.json(); if (!captureResponse.ok) { console.error("PayPal capture error", captureResponse.status, capture); throw new Error("PayPal could not capture the payment."); }
  ...
  const captureId = capture.purchase_units[0].payments.captures[0].id, amount = capture.purchase_units[0].payments.captures[0].amount?.value || "0.00", existingOrder = await env.DB.prepare("SELECT id, status FROM orders WHERE order_number = ? LIMIT 1").bind(customId).first(); if (!existingOrder) throw new Error("Internal order was not found.");
...
async function requireOwner(request, env) {
  ...
  await env.DB.prepare("UPDATE admin_sessions SET last_used_at=datetime('now') WHERE id=?").bind(admin.session_id).run(); return admin;
}
async function getOrder(request, env, includePrivate=false) {
  if (!env.DB) throw new Error("Database binding is not configured."); const number=new URL(request.url).searchParams.get("number")?.trim(); if(!number)throw new Error("Order number is required."); const order=await env.DB.prepare("SELECT id,order_number,status,subtotal,total,shipping_address_snapshot,customer_email_snapshot,customer_phone_snapshot,created_at,updated_at FROM orders WHERE order_number=? LIMIT 1").bind(number).first(); if(!order)return null; const items=await env.DB.prepare("SELECT product_name,variant_name,sku,size,dimensions,unit_price,quantity,line_total FROM order_items WHERE order_id=? ORDER BY rowid ASC").bind(order.id).all(); if(!includePrivate)return{order_number:order.order_number,status:order.status,subtotal:order.subtotal,total:order.total,created_at:order.created_at,updated_at:order.updated_at,items:items.results||[]}; return{...order,items:items.results||[]};
}
async function getAdminOrders(request, env) {
  if (!env.DB) throw new Error("Database binding is not configured."); const number=new URL(request.url).searchParams.get("number")?.trim();
  if(number){const order=await getOrder(request,env,true);return{success:true,orders:order?[order]:[]};}
