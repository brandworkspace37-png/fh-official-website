...
async function createPayPalOrder(request, env) {
  ...
  const now = new Date().toISOString(), existingCustomer = await env.DB.prepare("SELECT id FROM customers WHERE lower(email) = lower(?) LIMIT 1").bind(email).first("id"), customerId = existingCustomer?.id || crypto.randomUUID(), addressId = crypto.randomUUID(), orderId = crypto.randomUUID(), paymentId = crypto.randomUUID(), shippingSnapshot = JSON.stringify({ firstName, lastName, address, city, state, zip, country: "US" });
  const statements = [];
...
async function getOrder(request, env) { if (!env.DB) throw new Error("Database binding is not configured."); const number = new URL(request.url).searchParams.get("number")?.trim(); if (!number) throw new Error("Order number is required."); const order = await env.DB.prepare("SELECT id, order_number, status, subtotal, total, shipping_address_snapshot, customer_email_snapshot, customer_phone_snapshot, created_at, updated_at FROM orders WHERE order_number = ? LIMIT 1").bind(number).first(); if (!order) return null; const items = await env.DB.prepare("SELECT product_name, variant_name, sku, size, dimensions, unit_price, quantity, line_total FROM order_items WHERE order_id = ? ORDER BY rowid ASC").bind(order.id).all(); return { ...order, items: items.results || [] }; }
export default { async fetch(request, env) { const origin = getOrigin(request); if (request.method === "OPTIONS") return json({}, 204, origin || "*"); if (!origin) return json({ error: "Origin not allowed." }, 403, "*"); const url = new URL(request.url); try { if (url.pathname === "/api/health" && request.method === "GET") return json({ ok: true, db: Boolean(env.DB) }, 200, origin); if (url.pathname === "/api/project-leads" && request.method === "POST") return json(await saveProjectLead(request, env), 200, origin); if (url.pathname === "/api/paypal/create-order" && request.method === "POST") return json(await createPayPalOrder(request, env), 200, origin); if (url.pathname === "/api/paypal/capture-order" && request.method === "POST") return json(await capturePayPalOrder(request, env), 200, origin); if (url.pathname === "/api/orders" && request.method === "GET") { const order = await getOrder(request, env); return order ? json(order, 200, origin) : json({ error: "Order not found." }, 404, origin); } return env.ASSETS ? env.ASSETS.fetch(request) : new Response("Not found", { status: 404 }); } catch (error) { console.error(error); return json({ error: error instanceof Error ? error.message : "Unexpected server error." }, 400, origin); } } };
