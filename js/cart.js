document.addEventListener('DOMContentLoaded', () => {
    ...
    drawer.innerHTML = `
        <div class="cart-drawer-panel">
            <div class="cart-drawer-head"><div><span>FORM & HALO</span><h2>Tu carrito.</h2></div><button type="button" class="cart-close" aria-label="Cerrar carrito">×</button></div>
            <div class="cart-items"></div>
            <div class="cart-empty">Tu carrito está vacío.</div>
            <div class="cart-drawer-foot"><div><span>SUBTOTAL</span><strong class="cart-total">—</strong></div><button type="button" class="cart-checkout" disabled>Continuar al checkout</button></div>
        </div>`;
    ...
    const render = () => {
        ...
        itemsEl.innerHTML = cart.map((item, index) => `
            <div class="cart-item"><div><span>PRODUCTO 01</span><h3>${item.product}</h3><p>Tamaño: ${item.size}</p></div><div class="cart-item-controls"><button type="button" data-action="minus" data-index="${index}">−</button><b>${item.quantity}</b><button type="button" data-action="plus" data-index="${index}">+</button><button type="button" class="cart-remove" data-action="remove" data-index="${index}">Eliminar</button></div></div>`).join('');
